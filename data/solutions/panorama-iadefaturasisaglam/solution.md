# İade Faturası(Sağlam)

_Kaynak: Panorama Kullanım Kılavuzu_

**Menü Adımı:** `Satış ve Alış İşlemleri → Müşteriden İade İşlemleri → İade Faturası (Sağlam)`

## Ekran Tanımı

Müşteriden İade Faturaları (Sağlam) bu ekrandan girilir. Öncelikle ekrana, sisteme daha önceden girilmiş olan Müşteriden İade Faturaları (Sağlam) listelenir.
...
| Satış Temsilcisi | Müşteriden İade Faturası (Sağlam) hangi satış temsilcisi tarafından girildiğini gösteren alandır. Yukarıda seçilen müşteri bir temsilcisinin rutuna bağlı ise müşteri seçiminden sonra Satış Temsilcisi otomatik olarak gelir. Zorunlu bir alandır. |
| Hareket Tipi | Müşteriden İade Faturası (Sağlam) için tanımlanan hareket tipinin seçildiği alandır. Zorunlu bir alandır. |
...
| Onay Durumu | Eğer Müşteriden İade Faturası (Sağlam) için iş akış tanımı var ise Müşteriden İade Faturası (Sağlam) kaydedildiğinde onay durumu onaylanmadı olarak görünür. Kaydetmiş olduğumuz Müşteriden İade Faturası (Sağlam) İş akış onaydan onaylanması durumunda Belgenin Onay durumu onaylandı görünür. Fakat iş akış tanımı yok ise Müşteriden İade Faturası (Sağlam) ilk kaydedildiğinde onay durumu onaylandı olarak görünür. |
...
| Sipariş Takip Kodu | Müşteriden İade Faturası (Sağlam) herhangi bir siparişe bağlı olarak oluşmaz bu nedenden dolayı Müşteriden İade Faturası(Sağlam) belgelerinde bu alanda herhangi bir değer yer almaz. |
| İrsaliye Takip Kodu | Müşteriden İade Faturası (Sağlam) belgesine bağlı Müşteriden İade İrsaliyesi(Sağlam) var ise iade İrsaliyesinin belge kodu bu alanda görünür. |
