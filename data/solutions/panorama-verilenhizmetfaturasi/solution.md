...
**Menü Adımı:** `Satış ve Alış İşlemleri → Satış İşlemleri → Verilen Hizmet Faturası`

## Ekran Tanımı

Verilen Hizmet Faturaları bu ekrandan girilir. Öncelikle ekrana, sisteme daha önceden girilmiş olan Verilen Hizmet Faturaları listelenir.
...
| Satış Temsilcisi | Verilen Hizmet Faturasının hangi satış temsilcisi tarafından girildiğini gösteren alandır. Yukarıda seçilen müşteri bir temsilcisinin rutuna bağlı ise müşteri seçiminden sonra Satış Temsilcisi otomatik olarak gelir. Zorunlu bir alandır. |
| Hareket Tipi | Verilen Hizmet Faturası için tanımlanan hareket tipinin seçildiği alandır. Zorunlu bir alandır. |
...
| Depo Mik. | Verilen hizmet faturalarında sadece hizmet faturaları listelenir ve hizmet faturalarının stok bilgisi tutulmaz bundan dolayı seçmiş olduğumuz ürünün depo miktarı 0 görünecektir. |
...
| Onay Durumu | Eğer Verilen Hizmet Faturası için iş akış tanımı var ise Verilen Hizmet Faturası kaydedildiğinde onay durumu onaylanmadı olarak görünür. Kaydetmiş olduğumuz Verilen Hizmet Faturasının İş akış onaydan onaylanması durumunda Belgenin Onay durumu onaylandı görünür. Fakat iş akış tanımı yok ise Verilen Hizmet Faturası ilk kaydedildiğinde onay durumu onaylandı olarak görünür. |
| Yükleme Kod / Plaka | Verilen Hizmet Faturası yüklemeye dahil olmaz bu yüzden bu alanda her hangi bir değer yer almaz. |
...
| İrsaliye Takip Kodu | Verilen Hizmet Faturası irsaliye bağlı oluşmayacağı için bu alanda herhangi bir değer yer almaz. |
| Sipariş Takip Kodu | Verilen Hizmet Faturası siparişe bağlı oluşmayacağı için bu alanda herhangi bir değer yer almaz. |
...
| Döviz Tipi | Yerel Para birimden farklı bir para birimine ait ürün satışı yapılması durumunda Döviz tipi alanından hangi para birimine ait satış yapılacağı seçilir. Fakat verilen hizmet faturasında dövizli satış yapılmamaktadır. |
| Döviz Kuru | Seçilen Döviz Tipine ait döviz kuru seçilir. Fakat verilen hizmet faturasında dövizli satış yapılmamaktadır. |
| Döviz fiyatı | Dövizli fiyat alanıdır. Fakat verilen hizmet faturasında dövizli satış yapılmamaktadır. |
