...
**Menü Adımı:** `Satış ve Alış İşlemleri 🡪 Satış İşlemleri 🡪 Fatura`

## Ekran Tanımı

Satış Faturaları bu ekrandan girilir. Öncelikle ekrana, sisteme daha önceden girilmiş olan Satış Faturaları listelenir.
...
| Satış Temsilcisi | Faturanın hangi satış temsilcisi tarafından girildiğini gösteren alandır. Yukarıda seçilen müşteri bir temsilcisinin rutuna bağlı ise müşteri seçiminden sonra Satış Temsilcisi otomatik olarak gelir. Zorunlu bir alandır. |
| Hareket Tipi | Satış faturası için tanımlanan hareket tipinin seçildiği alandır. Zorunlu bir alandır. |
...
| Teslimat Yeri | Teslimat yeri seçimi yapılır. |
| İşlem Tarihi | Belgenin işlem tarihinin girildiği alandır. Default olarak günün tarihi gelir. Zorunlu bir alandır. |
| İşlem Saati | Faturanın kaydedildiği saat bilgisini gösterir. Bu alan pasiftir değiştirilemez. |
...
| Sevk Tarihi | Belgedeki ürünlerin sevk edileceği tarih bilgisi girilir. |
| Açık Kapalı | Belgenin Açık/Kapalı bilgisinin girildiği alandır. Ödemenin durumuna göre belirlenir. Ödemesi peşin alınacak ise kapalı seçilebilir. Kapalı seçilen faturanın otomatik olarak tahsilat kaydı da oluşturulur. Bu nedenle peşin alınan ya da alınması kesin belgeler için kapalı durumu seçilebilir. |
| Özel Kod | Zorunlu bir alan değildir. Özel kod değeri girilebilir. |
| Satış Tip | Satışın toptan mı parakende mi olduğunun belirtildiği alandır. |
...
| Giriş Tipi | Faturanın nereden girilmiş olduğunu belirtir. Panoramadan girilen faturalar için giriş tipi EnroutePanorama, Mobilden girilen faturaların giriş tipi PanoramaMobile, Ticari Paket aktarımı ile gelen faturaların giriş tipi Panorama Aktarım olarak görünür. |
| Belge Durumu | Faturanın durumunun ne olduğunu gösterir. Aktif, Pasif, İptal, Bloke, Silindi değerlerini alır. |
| Basım Durumu | Fatura kaydedildiğinde basım durumu yazdırılmadı olarak görünür. Fatura yazdırdıktan sonra basım durumu yazdırıldı olarak güncellenir. |
| Aktarım Durumu | Faturanın Ticari pakete aktarılıp aktarılmadığı bilgisini gösterir. Ticari pakete aktarılmamış Faturanın aktarım durumu Aktarılmadı, Ticari Pakete aktarılmış Faturanın aktarım durumu Aktarıldı olarak görünür. |
| Onay Durumu | Eğer Satış Faturası için iş akış tanımı var ise fatura kaydedildiğinde onay durumu onaylanmadı olarak görünür. Bu Faturanın İş akış onaydan onaylanması durumunda Belgenin Onay durumu onaylandı görünür. Fakat iş akış tanımı yok ise satış faturası ilk kaydedildiğinde onay durumu onaylandı olarak görünür. |
| Yükleme Kod / Plaka | Faturanın yüklemesi yapılmış ise bu alanda yükleme kodu ve yüklemedeki dağıtıcının plaka kodu bilgisi görünür. |
| Oluşturulma Yeri | Faturanın hangi menüden oluşturulduğunun bilgisinin gösterildiği alandır. |
| Oluşturulma Tarihi | Faturanın hangi tarih ve hangi saatte oluşturulduğunun gösterildiği alandır. |
| İrsaliye Takip Kodu | İlgili faturaya siparişten ya da irsaliyeden oluşturulmuş ise faturanın bağlı olduğu irsaliyenin belge kodu değeri görünür. |
| Sipariş Takip Kodu | İlgili fatura siparişten oluşturulmuş ise faturanın bağlı olduğu siparişin belge kodunu gösterir. |
| Komisyon TL | Faturada yer alan ürüne tanımlanmış Kredi kartı komisyonu tanımı var ise yapılan hesaplama sonucu fatura tutarına aktarılan komisyon tutarının gösterildiği alandır.(İade belgeleri için kullanılmamaktadır.) |
| Toplam Hacim/ Ağırlık | Fatura detayındaki ürünlerin toplam Hacim ve Ağırlık bilgisi değerleridir. Ürün kartı üzerindeki hacim ve ağırlık değerlerinden hesaplanır. |
| Toplama Durumu | Mobilden girilen Fatura depocu terminaline düşüyor ve depocu tarafından yapılan işlemin sonucu ile ilgili bilgi bu alandan görülür. |
| Brüt Tutar | Faturada yer alan ürünlerin, KDV tutarlarının toplam bilgisidir. |
...
| Yükleme Kodu | Kaydedilen Satış Faturası herhangi bir yüklemeye dahil değil ise ve herhangi bir yüklemeye dahil edilmek isteniyor ise Yükleme kodu alanından hangi yüklemeye dahil edileceği seçilir. |
| Yükleme Detay Kod | Satış Faturasının dahil edileceği yükleme kodu seçildikten sonra bu yüklemedeki hangi araca dahil edileceği Yükleme Detay Kodu alanından seçilir. |
...
| Takip Kodu | Bir belgeye birden fazla doküman eklenebilir ve her doküman için tanım yapılır yapılan tanımın takip kodu değeridir. |
| Dosya Tipi | Eklenecek olan dosyanın tipini, gösterir. |
| Dosya Uzantısı | Dosya eklendikten sonra satırda eklenen dosyanın uzantısını gösterir. |
| Depocuya Gönder | Faturadaki ürünlerin depocu tarafından toplanıp onaylanması için bu parametre işaretlenir. |
| Kısmi Toplama | Faturadaki ürünlerin kısmi olarak toplanması için bu parametre işaretlenir. |
...
| Kod | Girilen seri numarasına ait ürün bulunması durumunda bulunan ürünün kod değeri görünür. |
| Ad | Girilen seri numarasına ait ürün bulunması durumunda bulunan ürünün Ad değeri görünür. |
| Toplam Miktar | Seri miktarı ürün miktarını belirtir ve detaya eklenen seri miktarı toplam miktar değerini oluşur. |
| Tek Seri Girişi | Tek bir seriye ait ürün aratılacak ise tek seri girişi seçeneği işaretlenir. |
| Seri Numara Aralığı | Seri aralığı verilerek sıralı serilere ait ürün aratılmak istenmesi durumunda Seri Numara Aralığı seçeneği işaretlenir. |
| Seri Numarası | Seri başlangıç değeri girilir. |
| Seri Bitiş | Seri Bitiş değeri girilir. |
| Ekle | Seri değerleri girildikten sonra Ekle butonuna bastığımızda herhangi bir ürüne ait olması durumunda seriler detaya eklenir. |
| Çıkar | Detaya eklenen seriyi çıkartmak için kullanılır. |
| Temizle | Kriterleri ve detayı temizlemek için kullanılır. |
| Seç Kapat Butonu | Seriler detaya girildikten sonra Seç Kapat butonuna bastığımızda serilere ait bulunan ürünler belge detayına eklenir. |

## Buton Açıklamaları

| Alan | Açıklama |
| --- | --- |
| Yükleme | Faturanın daha önceden oluşturulmuş bir yüklemeye dahil edilmesinin istenmesi durumunda Yükleme butonundan işlem yapılır. |
| Tahsilat | Faturaya ait tahsilat girişi yapılır. |
| Dosya Ekleme | Belgeye ait dosya eklemek için Dosya Ekleme butonu kullanılır. |
...
| Sadakat | Müşterinin kazanılmış sadakat puanı var ise ve harcama dönemine girilmiş ise Sadakat harcaması bu adımdan yapılır. |
...
| Seriden Ürün Girişi | Belge detayına serili ürün eklenmesi durumunda, Seriden ürün girişi ekranından seri numaraları girilerek bu seri numaralarının hangi ürüne ait olduğu bulunur ve belge detayına eklenir. |
...
| Yenile | Eklenen dosya listesini yenilemek için kullanılır. |
| Yeni | Yeni dosya eklemek için tanım ekranını açar. |
| Düzenle | Eklenmiş olan dosya tanımını düzenlemek için kullanılır. |
| İzle | Eklenen dosya tanımını izlemek için kullanılır. |
| Sil | Eklenen dosya tanımını silmek için kullanılır. |
| Kapat | Dosya Ekleme ekranını kapatmak için kullanılır. |
